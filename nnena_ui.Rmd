...
### MONTH 12
observe({
  result=input$tx_gdycat
  # if they entered month 12, then there's no point in showing output
  if (is.element("mo12", result)) {
    output$mo12_sds <- renderText({ "\"come on, you already know this :)\"" })
    output$mo12_eqs <- renderText({ "\"come on, you already know this :)\"" })
  }
  else {
